...
fun MainScreen()
{
    ...
    TCServerSideImplementation.sendPageViewEvent(NavigationItem.Home.title, "view_page")
}
...
fun Navigation(navController: NavHostController)
{
    NavHost(navController, startDestination = NavigationItem.Home.route)
    {
        ...
        if ("ServerSide_Only" != BuildConfig.FLAVOR) {
            composable(NavigationItem.Banner.route) {
                Banner(navController)
            }
        }

        if (BuildConfig.FLAVOR == "Visualizer") {
            composable(NavigationItem.Visualizer.route) {
                JsonVisualizer(navController)
            }
        }
